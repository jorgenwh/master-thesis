...
However, the \textit{k}mer counting process in KAGE is slightly different from the process explained earlier in this section.
Rather than counting the observed frequency of every valid \textit{k}mer in a set of input reads, KAGE only counts the observed frequencies of every \textit{k}mer in a predetermined set.
Given how many valid \textit{k}mers one can observe in a set of (hundreds of?) millions of reads (\textbf{cite?}), which is typical when genotyping a human genome, not having to store each of these with an associated count value makes this new \textit{k}mer counting variant significantly less memory and time consuming.
...
\caption{
  \textit{Partial} \textit{k}mer counting where we only count the observed frequencies of \textit{k}mers present in a predetermined set. In this example, our set of predetermined \textit{k}mers is \{ATT, AGT, GTC, GAC\}. During counting, \textit{k}mers not present in this set are skipped.
}
